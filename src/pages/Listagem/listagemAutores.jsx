  ...
  return (
    ...
    >
      <div className="w-full flex items-center justify-center bg-black bg-opacity-60">
        <div className="bg-gradient-to-br from-red-600/20 via-pink-700/10 to-purple-800/20 backdrop-blur-md rounded-3xl p-6 w-full max-w-md text-white shadow-[0_0_25px_#f87171] border border-red-400">
          <h1 className="text-2xl font-bold text-center mb-4">Autores Cadastrados</h1>

          {loading ? (
            <p className="text-center">Carregando...</p>
          ) : autores.length ? (
            <ul className="space-y-3 max-h-80 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-red-500">
              {autores.map((autor, index) => (
                <li
                  key={index}
                  className="border border-white/30 rounded-lg p-3 flex justify-between items-center bg-white bg-opacity-10"
                >
                  <div>
